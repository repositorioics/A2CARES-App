 ...
 */
public class RecepcionLaboratorioListAdapter extends RecyclerView.Adapter<RecepcionLaboratorioListAdapter.ViewHolder> {

    private List<Serologia> listdata;
    private SimpleDateFormat mDateFormat = new SimpleDateFormat("MMM dd, yyyy");

    // RecyclerView recyclerView;
    public RecepcionLaboratorioListAdapter(List<Serologia> listdata) {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        final Serologia recepcionMuestra = listdata.get(position);
        holder.textViewIdent.setText(Constants.TIPO_TUBO_ROJO);
        holder.textViewIdent.setTextSize(20);
        holder.textViewIdent.setTextColor(Color.RED);
        holder.textViewDer.setText(mDateFormat.format(listdata.get(position).getFecha()));
        holder.textViewName.setText(holder.context.getString(R.string.code) + ": " + recepcionMuestra.getParticipante() + " - " + holder.context.getString(R.string.volumen) + ": " + recepcionMuestra.getVolumen() + holder.context.getString(R.string.ml));
    }
    ...
    public void filterList(ArrayList<Serologia> filteredList) {
