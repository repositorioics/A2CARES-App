...
/**
 * Created by Everts Morales on 08/04/2024.
 */
public class NuevoControlTemperaturaFragment extends Fragment {
    ...
    private ControlTemperaturaTermo nuevoControlAsistencia;
    ...
    private EditText tempTermo;

    Bundle datos = new Bundle();


    public static NuevoControlTemperaturaFragment create(String key) {
        NuevoControlTemperaturaFragment fragment = new NuevoControlTemperaturaFragment();
        Bundle args = new Bundle();
        args.putString(ARG_KEY, key);

        NuevoControlTemperaturaFragment f = new NuevoControlTemperaturaFragment();
        f.setArguments(args);
    ...
    public NuevoControlTemperaturaFragment() {}
    ...
    //    mImageButtonFif = (ImageButton) rootView.findViewById(R.id.imageButtonFif);
        ...
        tempTermo =  (EditText) rootView.findViewById(R.id.temperaturaTermo);
    ...
    public void afterTextChanged(Editable editable) {
        if (tempTermo.getText() != null && !tempTermo.getText().toString().isEmpty()){
            if (Double.parseDouble(tempTermo.getText().toString()) > 2 &&
                    Double.parseDouble(tempTermo.getText().toString())< 11){
                temperat = Double.parseDouble(tempTermo.getText().toString());
            }else{
                showToast(getActivity().getString(R.string.submit_valor_temperatura));
            }
        }else{
            showToast(getActivity().getString(R.string.submit_valor_temperaturaVacio));
        }
    ...
    private boolean validarEntrada() {
       ...
       Integer numero = (int) (Math.random() * 100) + 1;
        ...
        nuevoControlAsistencia = new ControlTemperaturaTermo();
            nuevoControlAsistencia.setFechatemp(date_asistencia);
        UUID deviceUuid = new UUID(infoMovil.getDeviceId().hashCode(), new Date().hashCode());

        if (tempTermo.getText() != null && !tempTermo.getText().toString().isEmpty()){
            if (Double.parseDouble(tempTermo.getText().toString()) > 1.9 &&
                    Double.parseDouble(tempTermo.getText().toString())< 10.1){
                temperat = Double.parseDouble(tempTermo.getText().toString());
                // nuevoControlAsistencia.setId(numero);
                // nuevoControlAsistencia.setUsuarioregistro(username) ;
                nuevoControlAsistencia.setHoratomatemp(horaentrada);
                nuevoControlAsistencia.setTemperaturaTermo(Double.parseDouble(tempTermo.getText().toString())); ;
                nuevoControlAsistencia.setUsuario(username);
    ...
    private class FetchCatalogosTask extends AsyncTask<String, Void, String> {
        ...
        private class SaveDataTask extends AsyncTask<String, Void, String> {
            ...
            protected String doInBackground(String... values) {
                try {
                    estudiosAdapter.open();
                    estudiosAdapter.crearControlTemperatura(nuevoControlAsistencia);
                } catch (Exception e) {
                    Log.e(TAG, e.getLocalizedMessage(), e);
                    return "error";
                }finally {
    ...
    private class SaveDataTask extends AsyncTask<String, Void, String> {
        ...
        protected void onPreExecute() {
            ...
            nDialog.setCancelable(false);
            nDialog.show();
            showToast(getActivity().getString(R.string.submit_confirm_button_asist_temp1));
        }
